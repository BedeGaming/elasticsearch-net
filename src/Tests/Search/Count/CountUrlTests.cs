	...
	public class CountUrlTests
	{
		[U] public async Task Urls()
		{
			...
			await GET("/_count")
				...
				.Request(c=>c.Count<Project>(new CountRequest<Project>(Best.Indices.All, Types.All)))
				.FluentAsync(c=>c.CountAsync<Project>(s=>s.AllIndices().Type(Types.All)))
				.RequestAsync(c=>c.CountAsync<Project>(new CountRequest<Project>(Best.Indices.All, Types.All)))
			...
			await POST("/_count")
				...
				.Request(c=>c.Count<Project>(new CountRequest<Project>(Best.Indices.All, Types.All) { Query = new MatchAllQuery() }))
				.FluentAsync(c=>c.CountAsync<Project>(s=>s.AllIndices().Type(Types.All).Query(q=>q.MatchAll())))
				.RequestAsync(c=>c.CountAsync<Project>(new CountRequest<Project>(Best.Indices.All, Types.All) { Query = new MatchAllQuery() }))
