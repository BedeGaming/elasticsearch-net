	...
	public class SearchExistsUrlTests
	{
		...
		public async Task Urls()
		{
			...
			await GET("/_search/exists")
				...
				.Request(c => c.SearchExists(new SearchExistsRequest<Project>(Best.Indices.All, Types.All)))
				.FluentAsync(c => c.SearchExistsAsync<Project>(s => s.AllIndices().Type(Types.All)))
				.RequestAsync(c => c.SearchExistsAsync(new SearchExistsRequest<Project>(Best.Indices.All, Types.All)))
			...
			await POST("/_search/exists")
				...
				.Request(c => c.SearchExists(new SearchExistsRequest<Project>(Best.Indices.All, Types.All) { Query = new MatchAllQuery() }))
				.FluentAsync(c => c.SearchExistsAsync<Project>(s => s.AllIndices().Type(Types.All).Query(q => q.MatchAll())))
				.RequestAsync(c => c.SearchExistsAsync(new SearchExistsRequest<Project>(Best.Indices.All, Types.All) { Query = new MatchAllQuery() }))
