	...
	public class GetWarmerUrlTests
	{
		[U] public async Task Urls()
		{
			...
			await GET($"/{index}/_warmer")
				.Fluent(c => c.GetWarmer(w=>w.Index(Best.Indices.Index(index))))
				.Request(c => c.GetWarmer(new GetWarmerRequest(Best.Indices.Index(index))))
				.FluentAsync(c => c.GetWarmerAsync(w=>w.Index(index)))
				.RequestAsync(c => c.GetWarmerAsync(new GetWarmerRequest(Best.Indices.Index(index))))
				;
